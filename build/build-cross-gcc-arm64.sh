...
INSTALL_PATH=/opt/cross/armv8-gnu/
TARGET=arm-unknown-linux-gnueabihf
USE_NEWLIB=0
LINUX_ARCH=arm
# libmpx doesn't compile - PATH_MAX not defined
CONFIGURATION_OPTIONS="--disable-multilib --disable-libmpx --with-arch=armv8-a --with-fpu=vfpv3-d16 --with-float=hard -march=armv8-a+simd -mcpu=cortex-a72+simd"
...
# Extract everything
for f in *.tar*; do bsdtar xfk $f; done
...
if [ $USE_NEWLIB -ne 0 ]; then
    ...
    mkdir -p build-glibc
    cd build-glibc
    ../$GLIBC_VERSION/configure --prefix=$INSTALL_PATH/$TARGET --build=$MACHTYPE --host=$TARGET --target=$TARGET --with-headers=$INSTALL_PATH/$TARGET/include $CONFIGURATION_OPTIONS libc_cv_forced_unwind=yes libc_cv_ssp=no
